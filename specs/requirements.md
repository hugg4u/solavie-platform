...
7. THE Gateway SHALL trích xuất claims từ JWT đã xác minh (tenant_id, sub, roles) và inject thành headers nội bộ: X-Tenant-ID, X-User-ID, X-User-Roles trước khi forward tới downstream services
...
4. THE Auth_Service SHALL cho phép Tenant Admin tạo custom roles và gán permissions từ danh mục có sẵn (Dynamic RBAC)
...
7. WHEN Tenant Admin thay đổi permissions của một Role, THE Auth_Service SHALL thu hồi ngay lập tức cached session liên quan trên Redis trong vòng < 1 giây
...
10. THE Auth_Service SHALL kiểm tra permission từ Redis cache trong < 0.05 giây; cache miss mới truy vấn DB
...
9. THE Platform SHALL ngăn chặn MCP Server của tenant này truy cập dữ liệu của tenant khác qua Roots boundary configuration
