...
#### Acceptance Criteria
1. THE Downstream Services SHALL tự phát hiện IP nội bộ của mình khi khởi chạy và đăng ký vào Redis Set `registry:service:{service_name}`.
2. THE Downstream Services SHALL gửi tin nhắn sống (heartbeat) mỗi 5 giây lên Redis key `registry:service:{service_name}:node:{ip}:{port}` với thời gian sống (TTL) 15 giây.
3. THE Downstream Services SHALL tự động xóa IP của mình khỏi Redis khi nhận tín hiệu kết thúc (`SIGTERM` hoặc `SIGINT`).
4. THE Gateway Service SHALL chạy một luồng nền (Registry Sync Daemon) để đồng bộ danh sách IP từ Redis vào các Target của Kong Upstream tương ứng qua Kong Admin API.
5. THE Gateway Service SHALL cấu hình active/passive health checks và tự động thử lại (`retries: 5`) trên Kong Upstream để tự động loại bỏ các IP chết và chuyển hướng request thông suốt.
