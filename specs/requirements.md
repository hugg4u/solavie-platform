...
10. THE Gateway SHALL vận hành ở chế độ DB Mode kết nối PostgreSQL (schema `kong_db`) và cho phép reload/cập nhật target động qua REST Admin API để đảm bảo High Availability.
...
2. THE Auth_Service SHALL phát hành JWT Access Token (TTL 15 phút) và Refresh Token (TTL 7 ngày) chứa claims: organization (id, name, roles), tenant_id, sub (user_id)
3. THE Auth_Service SHALL hỗ trợ các vai trò mặc định (Org Roles) ở mức Organization: admin, manager, agent, viewer
...
5. THE Auth_Service SHALL hỗ trợ multi-tenant isolation qua Keycloak Organizations trong shared Realm 'solavie'
...
8. WHEN Super Admin tạo Tenant mới, THE Auth_Service SHALL tự động khởi tạo Organization mới, thiết lập default Org Roles và tạo tài khoản Admin đầu tiên
...
7. THE Platform SHALL cô lập Keycloak Organizations per tenant để đảm bảo identity isolation
...
### Requirement 13: Infrastructure-Agnostic Service Discovery (MỚI)

**User Story:** Là một system architect, tôi muốn hệ thống tự động phát hiện và đồng bộ IP của các microservice nghiệp vụ một cách độc lập với hạ tầng triển khai để tránh downtime khi deploy phiên bản mới.

#### Acceptance Criteria
1. THE Downstream Services SHALL tự phát hiện IP nội bộ của mình khi khởi chạy theo độ ưu tiên: Biến môi trường `CONTAINER_IP` > Quét các interface card mạng vật lý của OS > Fallback kết nối UDP fake đến `8.8.8.8`.
2. THE Downstream Services SHALL đăng ký địa chỉ `IP:Port` của mình vào Redis Set `registry:service:{service_name}` khi startup.
3. THE Downstream Services SHALL gửi tin nhắn sống (heartbeat) mỗi 5 giây lên Redis key `registry:service:{service_name}:node:{ip}:{port}` với thời gian sống (TTL) 15 giây.
4. THE Downstream Services SHALL tự động xóa IP của mình khỏi Redis Set và xóa key TTL khi nhận tín hiệu kết thúc (`SIGTERM` hoặc `SIGINT`) để đảm bảo Graceful Deregistration.
5. THE Downstream Services SHALL tích hợp cơ chế Fail-Safe: Registry client không crash ứng dụng nếu Redis tạm thời mất kết nối khi khởi chạy.
6. THE Downstream Services SHALL cung cấp endpoint `/health` (hoặc `/healthz`) trả về HTTP 200 OK để phục vụ Active Healthcheck.
7. THE Gateway Sync Daemon SHALL hoạt động ở chế độ bất đồng bộ (Asynchronous Python) quét định kỳ Redis và gọi trực tiếp REST Admin API của Kong (`POST/DELETE /upstreams/{upstream}/targets`) để cập nhật target động tức thời mà không gây ảnh hưởng đến cấu hình chung.
8. THE Gateway SHALL áp dụng active và passive health checks trên Upstream kết hợp cơ chế `retries` (tối thiểu 5 lần) để tự động ngắt kết nối đến node sập dưới 1 giây.

### Requirement 14: RAG Quality & Knowledge Analytics System (MỚI)

**User Story:** Là chủ doanh nghiệp, tôi muốn giám sát chất lượng trả lời RAG của trợ lý ảo và phát hiện các khoảng trống tri thức để liên tục nâng cấp tài liệu doanh nghiệp.

#### Acceptance Criteria
1. THE Downstream Services SHALL truyền tải chỉ số `max_similarity_score` (phản ánh độ tương đồng RAG cao nhất sau rerank) và `handoff_reason` qua các gRPC calls giữa Chatbot và AI Core.
2. THE AI Core SHALL publish sự kiện `chatbot.conversation.completed` lên Kafka topic khi hội thoại kết thúc, chứa đầy đủ metadata (event_id, tenant_id, user_query, standalone_query, query_rewritten, max_similarity_score, grounding_score, chatbot_action, handoff_reason, latency_ms).
3. THE Analytics Service SHALL consume sự kiện trên, kiểm soát trùng lặp (idempotency) qua `event_id`, và lưu trữ vào TimescaleDB hypertable `rag_metrics`.
4. THE Analytics Service SHALL cung cấp API `/api/v1/knowledge-gaps` tổng hợp và nhóm các câu hỏi có `max_similarity_score < 0.50` hoặc `handoff_reason == 'rag_no_docs_found'` theo ngữ nghĩa để phát hiện khoảng trống tri thức (Knowledge Gap).
5. THE Analytics Service SHALL cung cấp API `/api/v1/rag-performance` tính toán hiệu năng RAG trung bình (avg similarity, grounding score, cache hit rate) của tenant theo khung thời gian.
6. THE RAG Quality APIs SHALL được bảo vệ bởi HMAC Signature Verification (sử dụng `GATEWAY_SIGNING_SECRET`) và Dynamic RBAC với quyền yêu cầu là `analytics:metrics:read`.
7. THE Analytics Service SHALL cache kết quả Knowledge Gaps trên Redis trong 5 phút để tránh các truy vấn SQL nặng liên tục lên TimescaleDB.
