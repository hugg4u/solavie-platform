...
7. THE Gateway SHALL trích xuất claims từ JWT đã xác minh (tenant_id, sub, roles), tự động phân giải chúng thành danh sách quyền hạn động theo quy chuẩn `{service}:{resource}:{action}` thông qua 2 lớp cache (Redis và Kong Shared Memory) kết hợp API Fallback gọi tới Tenant Config Service. Gateway SHALL ký số danh sách quyền này bằng HMAC-SHA256 sử dụng khóa bí mật chung và inject thành các headers bảo mật: X-Tenant-ID, X-User-ID, X-User-Permissions (chuỗi CSV) và X-Permissions-Signature trước khi forward tới downstream services.
