...
- [x] Triển khai logic luồng khóa/mở khóa (gọi Keycloak Admin API cập nhật trạng thái `"enabled": false/true`).
  - [x] Tích hợp gọi Keycloak API Force Logout `POST /admin/realms/{realm}/users/{id}/logout` để hủy các sessions hiện tại của user bị khóa.
...
- [x] Triển khai các REST API gán và thu hồi vai trò tùy chỉnh cho người dùng (`POST /api/v1/users/:id/roles`, `DELETE /api/v1/users/:id/roles/:name`) để gán vai trò của người dùng trên Keycloak Realm.
- [x] Triển khai các REST API Proxy tạo và xóa vai trò tùy chỉnh (`POST /api/v1/users/roles`, `DELETE /api/v1/users/roles/:name`) để làm Auth Proxy trung gian đồng bộ Realm Roles lên Keycloak Realm `solavie` cho Tenant Config Service gọi sang.
- [ ] **[BẢO MẬT]** Tái cấu trúc cơ chế lấy Admin Token: Chuyển đổi từ Password Grant Flow (sử dụng tài khoản master admin) sang Client Credentials Flow sử dụng client chuyên dụng `user-service-client` và biến môi trường `KEYCLOAK_CLIENT_SECRET` trong realm `solavie` để tuân thủ nguyên tắc Least Privilege.
...
- [ ] Bổ sung các test cases kiểm tra Signature Verification, Master Realm check bypass, gán từ khóa bảo lưu, và Access Control Denied.
