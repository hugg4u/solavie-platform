...
- [x] Triển khai NestJS `PermissionsGuard` phân giải quyền hạn in-memory O(1) hỗ trợ wildcard (`*`, `user:*`, `user:{resource}:*`).
  - [x] **[CRITICAL]** Bổ sung cơ chế bảo vệ chống Privilege Escalation: Chỉ cho phép tự động gán wildcard `*` cho role `system`/`system_admin` nếu `tenant_id` trùng khớp với Master Realm (`solavie-system-master`).
