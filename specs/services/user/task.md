...
- [x] Triển khai logic luồng khóa/mở khóa (gọi Keycloak Admin API cập nhật trạng thái `"enabled": false/true`).
  - [x] Tích hợp gọi Keycloak API Force Logout `POST /admin/realms/solavie/users/{id}/logout` to invalidate current user sessions.
  - [x] Triển khai publish sự kiện `token.revoked` (kèm các `jti`) lên **Apache Kafka topic `token.revoked`** để Auth Sync Worker nhận và cập nhật vào Redis Blacklist.
...
### Task: Tích hợp Apache Kafka Client
- [ ] Triển khai Kafka client (sử dụng `@nestjs/microservices` hoặc `kafkajs`) kết nối với Broker.
- [ ] Triển khai Kafka Producer để publish sự kiện hủy token sang topic `token.revoked`.
- [ ] Triển khai Kafka Producer để publish sự kiện gửi email mời sang topic `notification.send`.
- [ ] Triển khai Kafka Consumer lắng nghe topic `auth.events.user` thuộc consumer group `user-identity-sync-group` để nhận và đồng bộ các thay đổi từ Keycloak.
