...
User Service giữ vai trò độc quyền nắm giữ Client Credentials có quyền quản trị (`user-service-client`) trên shared realm 'solavie'. 
...
*   **Ánh xạ tên vai trò:** Khi tạo vai trò tùy chỉnh trên Keycloak, User Service tự động thêm prefix `tenant_id:` vào tên vai trò để cô lập danh không gian vai trò giữa các tenant sử dụng chung Organization trong shared realm.
...
1.  **Giai đoạn Khởi tạo:**
    ...
    *   User Service thay mặt gọi Keycloak Admin API để tạo một tài khoản "Shadow" (trạng thái `"enabled": false`) với Email được mời và liên kết tài khoản này vào Organization tương ứng của Tenant trong shared realm. Nhận về Keycloak User UUID và lưu khớp vào khóa chính `id` của bảng `users`.
2.  **Giao tiếp & Kích hoạt:**
    *   User Service sinh một Token kích hoạt dùng một lần có thời gian hết hạn nghiêm ngặt (TTL 24 giờ).
    *   Gửi sự kiện yêu cầu gửi email mời (chứa email, link kích hoạt kèm token) sang **Apache Kafka topic `notification.send`** để `Notification Service` nhận và xử lý gửi mail cho nhân viên.
    *   Nhân viên nhấp vào link, thiết lập mật khẩu trên Keycloak giao diện. Keycloak kích hoạt tài khoản, liên kết vào Organization và xác minh email.
3.  **Lazy Synchronization (Tự phục hồi đồng bộ):**
    *   Trong trường hợp event từ Keycloak (qua Auth Sync Worker) gửi về User Service bị trễ hoặc thất lạc do sự cố mạng, hệ thống áp dụng cơ chế tự phục hồi: Khi người dùng đăng nhập lần đầu tiên thành công và gửi request kèm JWT hợp lệ đến User Service, nếu trạng thái của User trong cơ sở dữ liệu local vẫn là `PENDING`, User Service sẽ tự động cập nhật trạng thái User thành `ACTIVE` ngay lập tức để không cản trước trải nghiệm của người dùng.
...
*   **Hủy bỏ Sessions:** User Service gọi Keycloak Admin API gửi yêu cầu Force Logout (`POST /admin/realms/solavie/users/{id}/logout`) để hủy lập tức các token/session đang hoạt động của user đó trên Keycloak.
*   **Thu hồi tức thời tại Gateway (JTI Blacklisting):** User Service publish sự kiện `token.revoked` (kèm UUID người dùng và danh sách `jti`) lên **Apache Kafka topic `token.revoked`**. `Auth Sync Worker` consume sự kiện này và ghi nhận `jti` vào Redis Blacklist, giúp API Gateway (Kong) ngay lập tức từ chối mọi yêu cầu mang token cũ trong vòng `< 1ms`.
