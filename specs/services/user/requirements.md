...
4. THE USER_Service SHALL thực hiện kiểm tra quyền in-memory O(1) dựa trên HTTP Header `X-User-Permissions` truyền từ Gateway. Định dạng quyền của dịch vụ tuân theo cấu trúc `user:{resource}:{action}` hỗ trợ ký tự đại diện `*` (Super Admin), `user:*` (Toàn quyền trên service), và `user:{resource}:*` (Toàn quyền trên tài nguyên).
