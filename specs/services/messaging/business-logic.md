...
- Dịch vụ trích xuất các headers từ request:
  ...
  - `X-User-Permissions`: Chuỗi CSV chứa danh sách quyền của người dùng (ví dụ: `messaging:{resource}:{action}`).
...
- Đối với mỗi API endpoint yêu cầu quyền hạn `messaging:{resource}:{action}`, dịch vụ kiểm tra quyền trong Set:
  - Nếu Set chứa `*` (Super Admin), cho phép truy cập.
  - Nếu Set chứa `messaging:*` (Toàn quyền trên dịch vụ), cho phép truy cập.
  - Nếu Set chứa `messaging:{resource}:*` (Toàn quyền trên tài nguyên), cho phép truy cập.
  - Nếu Set chứa chính xác `messaging:{resource}:{action}`, cho phép truy cập.
...
* BƯỚC 3: Thực hiện pipeline ghi vào Redis:
  * `SADD registry:service:messaging "{ip}:{port}"`
  * `SETEX registry:service:messaging:node:{ip}:{port} 15 "alive"`
...
* BƯỚC 2: Thực hiện pipeline dọn dẹp Redis:
  * `SREM registry:service:messaging "{ip}:{port}"`
  * `DEL registry:service:messaging:node:{ip}:{port}"`
