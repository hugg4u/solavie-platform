...
Dịch vụ kiểm tra và xác thực chữ ký signature trên mỗi request tại lớp Guard/Interceptor của Go / Prometheus / Grafana:
...
4. So khớp in-memory O(1): parse `X-User-Permissions` thành một Set và đối chiếu với quyền yêu cầu của endpoint (ví dụ: `observability:metrics:read`).
   - Hỗ trợ wildcard: `*` (Super Admin bypass), `observability:*` (Service bypass), và `observability:metrics:*` (Resource bypass).

## Security & Gateway Integration
- Dịch vụ được triển khai stateless phía sau Kong API Gateway.
- Gateway chịu trách nhiệm validate JWT token từ Keycloak, xác thực client scope `observability`, và inject header `X-Tenant-ID` vào request.
