...
### Acceptance Criteria
1. THE Comment Manager Service SHALL tự động phát hiện IP nội bộ của card mạng chính khi khởi động bằng cơ chế socket UDP ảo.
2. THE Comment Manager Service SHALL đăng ký địa chỉ `IP:Port` của mình vào Redis Set `registry:service:comment-manager` khi startup.
3. THE Comment Manager Service SHALL gửi tin nhắn sống (heartbeat) định kỳ mỗi 5 giây lên Redis key `registry:service:comment-manager:node:{ip}:{port}` với TTL là 15 giây.
4. THE Comment Manager Service SHALL dọn dẹp (hủy đăng ký) thông tin của mình trên Redis Set `registry:service:comment-manager` và xóa key TTL khi nhận tín hiệu shutdown (`SIGTERM`/`SIGINT`).
