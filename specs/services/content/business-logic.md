...
* BƯỚC 3: Thực hiện pipeline ghi vào Redis:
  * `SADD registry:service:content "{ip}:{port}"`
  * `SETEX registry:service:content:node:{ip}:{port} 15 "alive"`
...
* BƯỚC 2: Thực hiện pipeline dọn dẹp Redis:
  * `SREM registry:service:content "{ip}:{port}"`
  * `DEL registry:service:content:node:{ip}:{port}"`
