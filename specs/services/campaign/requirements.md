...
### Acceptance Criteria
1. THE Campaign Service SHALL tự động phát hiện IP nội bộ của card mạng chính khi khởi động bằng cơ chế socket UDP ảo.
2. THE Campaign Service SHALL đăng ký địa chỉ `IP:Port` của mình vào Redis Set `registry:service:campaign` khi startup.
3. THE Campaign Service SHALL gửi tin nhắn sống (heartbeat) định kỳ mỗi 5 giây lên Redis key `registry:service:campaign:node:{ip}:{port}` với TTL là 15 giây.
4. THE Campaign Service SHALL dọn dẹp (hủy đăng ký) thông tin của mình trên Redis Set `registry:service:campaign` và xóa key TTL khi nhận tín hiệu shutdown (`SIGTERM`/`SIGINT`).
