...
| 1.5.0 | 12/06/2026 | BA Team | **Bổ sung nghiệp vụ Query Rewriting và RAG Quality & Knowledge Gaps:** Tích hợp cơ chế tự động tối ưu hóa câu hỏi đa lượt (Query Rewriter) để giải quyết ngữ cảnh cuộc chat và thiết lập quy trình giám sát chất lượng RAG, phát hiện khoảng trống tri thức. |
...
*   **Cơ chế Tự động tối ưu hóa câu hỏi (Query Rewriting):**
    Để duy trì ngữ cảnh mà không gây bùng nổ token hoặc tăng tỷ lệ ảo tưởng (hallucination), hệ thống áp dụng cơ chế tự động viết lại câu hỏi:
    - Khi phiên chat có độ dài lịch sử lớn hơn hoặc bằng 2 tin nhắn, hệ thống sẽ sử dụng LLM để viết lại câu hỏi cuối cùng của người dùng thành một **Câu hỏi Độc lập (Standalone Query)** tự chứa đủ ngữ cảnh.
    - Câu hỏi độc lập này sẽ được dùng làm query đầu vào cho việc tìm kiếm thông tin RAG từ Vector DB để tăng độ chính xác kết quả trả về.
    - Kết quả viết lại câu hỏi được cache tạm thời trong Redis Cache (TTL 1 giờ) nhằm tối ưu chi phí API. Trường hợp LLM viết lại bị lỗi, hệ thống tự động fallback sử dụng nguyên văn câu hỏi gốc của khách hàng để đảm bảo tính sẵn sàng cao.
...
### 3.12. Giám sát Chất lượng RAG & Phát hiện Khoảng trống Tri thức (RAG Quality & Knowledge Gaps)
Để nâng cao chất lượng phục vụ của trợ lý AI và bổ sung tài liệu tri thức doanh nghiệp một cách chính xác, hệ thống thiết lập quy trình phân tích và giám sát chất lượng:
*   **Thu thập Chỉ số RAG (RAG Metrics Ingestion):**
    - Sau mỗi lượt chat hoàn thành, AI Core sẽ phát tín hiệu sự kiện bất đồng bộ qua Kafka topic `chatbot.conversation.completed` (chế độ fire-and-forget để bảo vệ thời gian phản hồi chat).
    - Dữ liệu sự kiện lưu giữ: độ tương đồng tri thức (`rag_similarity_score`), độ trung thực của câu trả lời (`nli_grounding_score`), hành động thực tế (`reply`, `handoff`, `clarify`, `lead_capture`), trạng thái trúng cache và lý do chuyển giao (nếu có).
*   **Phát hiện Khoảng trống Tri thức (Knowledge Gap Discovery):**
    - Hệ thống tự động lọc các câu hỏi có độ tương đồng thấp (similarity < 0.50) hoặc các phiên chat bị chuyển giao (handoff) sang Agent do thiếu tri thức.
    - Tiến hành gom nhóm ngữ nghĩa (semantic clustering) để hiển thị danh sách các câu hỏi phổ biến mà chatbot chưa trả lời tốt trên Dashboard của Admin.
    - Quản trị viên (Admin) dựa vào báo cáo này để cập nhật kịp thời tài liệu, lấp đầy khoảng trống tri thức của doanh nghiệp.
