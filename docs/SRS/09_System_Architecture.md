...
| 2 | **Auth** | Java | Keycloak 26+ | 8080 | keycloak_db (Postgres) | Xác thực OIDC, quản lý Organizations đa tenant trong shared Realm 'solavie', phân quyền RBAC. |
...
2. **Thay đổi vai trò và phân quyền**: 
   - **Tạo/Xóa Role**: Tenant Admin thao tác trên Dashboard. `Tenant Config Service` cập nhật PostgreSQL, gọi Keycloak Admin API để đồng bộ tạo/xóa Organization-scoped Role tương ứng trên Keycloak Organization.
...
## 9.5. Cơ chế Phát hiện Dịch vụ Động (Dynamic Service Discovery & Routing)

Để đạt tính độc lập tuyệt đối với hạ tầng triển khai (Infrastructure-Agnostic) và tránh các lỗi trễ/phụ thuộc vào DNS Caching của hệ điều hành hay Docker DNS khi triển khai bản phát hành mới, hệ thống Solavie sử dụng cơ chế định tuyến động tự xây dựng dựa trên Redis Registry và custom Lua plugin của Kong Gateway:

### 9.5.1. Sơ đồ tuần tự phát hiện dịch vụ:
```mermaid
sequenceDiagram
    participant ServiceNode as Backend Service Instance
    participant Redis as Redis Registry (Shared Store)
    participant Kong as Kong Gateway (dynamic-policy)

    Note over ServiceNode: Khởi chạy node mới
    ServiceNode->>ServiceNode: Tự phát hiện IP nội bộ & Port
    ServiceNode->>Redis: SADD registry:service:name ip:port
    
    loop Định kỳ mỗi 5 giây
        ServiceNode->>Redis: SETEX registry:service:name:node:ip:port 15 "alive"
    end

    Note over Kong: Client request gửi đến Gateway
    Kong->>Kong: Kiểm tra Nginx L1 Cache (perm_cache)
    alt Cache Miss (quá 2 giây)
        Kong->>Redis: SMEMBERS registry:service:name
        Redis-->>Kong: Danh sách các nodes hoạt động
        Kong->>Kong: Lọc các node có heartbeat "alive"
        Kong->>Kong: Lưu vào Nginx L1 Cache (TTL: 2s)
    end
    Kong->>Kong: Cân bằng tải (Round-Robin / Random)
    Kong->>Kong: kong.service.set_target(ip, port)
    Kong->>ServiceNode: Chuyển tiếp request trực tiếp tới IP:Port
```

### 9.5.2. Quy trình Vận hành Chi tiết:
1. **Service Registration (Đăng ký dịch vụ):** Khi một bản sao (instance) của bất kỳ backend service nào khởi chạy, nó tự phát hiện IP nội bộ của nó và đăng ký vào một Redis Set: `registry:service:{service_name}` với giá trị `ip:port`. Đồng thời, một luồng nền chạy heartbeat mỗi 5 giây gán khóa tạm thời `registry:service:{service_name}:node:{ip}:{port}` với TTL là 15 giây.
2. **Service Deregistration (Hủy đăng ký):** 
   - *Graceful Shutdown:* Khi nhận tín hiệu tắt (`SIGTERM`/`SIGINT`), instance tự gỡ bỏ IP khỏi Redis Set và xóa khóa heartbeat.
   - *Hard Crash:* Nếu instance sụp đổ đột ngột, sau 15 giây không có heartbeat, khóa sống trên Redis tự động hết hạn, giúp Kong Gateway tự loại bỏ node chết ra khỏi danh sách định tuyến.
3. **Dynamic Routing (Định tuyến động tại API Gateway):**
   - Plugin Lua `dynamic-policy` trên Kong API Gateway can thiệp vào pha `access`.
   - Để tránh nghẽn Redis, danh sách các node hoạt động được lưu vào bộ nhớ dùng chung của Nginx (L1 Cache - `ngx.shared.perm_cache`) trong 2 giây.
   - Khi cache hết hạn (Cache Miss), Gateway truy vấn Redis để lấy các node sống, lưu lại vào L1 Cache, thực hiện Load Balancing (xoay vòng hoặc ngẫu nhiên) và gọi `kong.service.set_target(ip, port)` để định tuyến trực tiếp.
   - *Cơ chế Fallback:* Nếu Redis gặp sự cố hoặc không tìm thấy node nào hoạt động, Gateway sẽ tự động fallback về tên DNS mặc định (`{service_name}:8000`) để bảo vệ hệ thống khỏi gián đoạn hoàn toàn.

---

## 9.6. Chiến lược Multi-tenancy & Cô lập dữ liệu (Data Isolation)
...
1. **Identity & Access Isolation (Zero-Trust HMAC Header):** Sử dụng tính năng Keycloak Organizations trong shared Realm 'solavie' (từ phiên bản Keycloak 26+). Mỗi Tenant khi đăng ký sẽ được khởi tạo một Organization riêng biệt chứa danh sách User, Roles và Client Credentials độc lập. Token JWT phát hành chứa claim `tenant_id` đại diện cho ID của Organization và danh sách `roles` được ánh xạ tương ứng.
   ...
   * **Deterministic Caching:** Quyền được sắp xếp theo thứ tự bảng chữ cái để tạo tính nhất quán. Vai trò `admin` nội bộ của tenant tự động được phân giải thành wildcard `*` (giới hạn bởi tenant_id). Vai trò `system` hoặc `system_admin` chỉ được phân giải thành wildcard `*` và cho phép bypass nếu `tenant_id` từ JWT trùng khớp với Master Tenant ID (`solavie-system-master`). Nếu vai trò này được gán dưới vai trò thuộc Organization thông thường, Gateway chặn đứng và trả về lỗi `403 Forbidden` để ngăn chặn Privilege Escalation.
...
## 9.7. Mô hình triển khai hạ tầng (Deployment Diagram)
...
## 9.8. Kế hoạch triển khai và phát hành (Deployment Phases)
