...
7. **Phân tích hiệu quả (Analytics):** Báo cáo hiệu suất Agent, Chatbot, chiến dịch marketing, ROI và **giám sát chất lượng RAG/phát hiện khoảng trống tri thức (Knowledge Gaps)**.
...
- Chatbot AI với RAG, **Query Rewriting**, Vision (đọc ảnh hóa đơn), MCP tools
...
- Real-time Analytics & Dashboard (bao gồm RAG Quality Monitoring & Knowledge Gaps)
...
| **Query Rewriter** | Bộ tối ưu hóa câu hỏi — thành phần ngôn ngữ tự động viết lại các câu hỏi phụ thuộc lịch sử hội thoại thành câu hỏi độc lập (Standalone Query). |
| **Knowledge Gap** | Khoảng trống tri thức — các chủ đề, câu hỏi của khách hàng mà hệ thống RAG không có tài liệu tri thức phù hợp để trả lời hoặc bị handoff sang Agent. |
| **Grounding Score** | Điểm chứng thực phản hồi — chỉ số đánh giá xem câu trả lời của LLM có hoàn toàn dựa trên tài liệu tri thức (RAG context) hay không (chống ảo tưởng). |
