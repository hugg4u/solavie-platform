...
Dưới đây mô tả chi tiết sơ đồ thành phần cấu trúc nội bộ và luồng gọi dịch vụ giữa `Messaging Service`, `Chatbot Service` và `AI Core Service` tích hợp các lớp tối ưu hóa cùng hệ thống MCP Server động tại `CRM Service`:
...
graph TD
    ...
    subgraph "CRM Service (NestJS)"
        crm_mcp[CRM MCP Controller - SSE]
        solar_mcp[Solar Calc MCP Controller - SSE]
        om_mcp[OM Ticket MCP Controller - SSE]
        crm_db[(CRM PostgreSQL DB)]
    end

    subgraph "Infrastructure Layer"
        redis_stack[(Redis Stack - DB 0 Cache)]
        qdrant[(Qdrant Vector DB)]
    end
    ...
    ai_core_server -->|9. Check Semantic Cache| cache_mgr
    cache_mgr -->|9a. Search Vector KNN| redis_stack
    
    %% Luồng Cache Hit
    cache_mgr -.->|Cache Hit < 10ms| ai_core_server
    
    %% Luồng Cache Miss
    cache_mgr -->|9b. Cache Miss| input_guard
    input_guard -->|10. Pass| llm_router
    llm_router -->|11. Query Context| kb_service
    kb_service --> qdrant
    llm_router -->|12. Validate Grounding| output_guard
    llm_router -->|13. Call Whitelisted Tool| mcp_host
    
    %% Luồng MCP Calls đi qua Kong Gateway tới CRM Service
    mcp_host -->|14. SSE Call via Kong| solar_mcp & crm_mcp & om_mcp
    solar_mcp & crm_mcp & om_mcp --> crm_db
```
...
6.  **Multi-tenant MCP Host Gateway:** Đăng ký và quản lý kết nối MCP Server động theo Tenant ID trong `config_db` (dùng giao thức SSE và bảo vệ bằng mTLS/OAuth 2.1). Phân giải và khởi chạy phiên kết nối MCP Client Session độc lập khi có yêu cầu gọi công cụ của Tenant đó. Chỉ kết nối tới các Custom MCP Server nội bộ (như `solar_calc`, `crm`, `om_ticket`) đã được whitelisting. Tự động ghi đè/tiêm tham số `tenant_id` từ JWT xác thực vào mọi tool call để cô lập dữ liệu.
7.  **Roots Security Boundary:** Tự động truyền giới hạn thư mục ranh giới `Roots` dựa trên Tenant ID khi kết nối MCP Server và khóa chặn các kết nối tới public/external MCP server công cộng không an toàn để triệt tiêu nguy cơ SSRF, SQL Injection và rò rỉ thông tin.
...
10. **Tự động Khởi tạo Cấu hình Định tuyến (Auto-configuration on First Key):** Tự động sinh ra 5 bản ghi cấu hình định tuyến (`LLMRouteConfig`) cho cả 5 usecases của Tenant ngay khi phát hiện Tenant thêm hoặc đồng bộ khóa API hoạt động đầu tiên. Các định tuyến này kế thừa giá trị mặc định của Provider tương ứng từ bảng default `system_default_route_configs` trong DB.
11. **API `/models` Danh mục Mô hình Động từ Registry:** Cung cấp API endpoint `/api/v1/completions/models` trả về danh sách mô hình hoàn toàn động bằng cách duyệt qua registry LiteLLM (`model_cost`) của 12 nhà cung cấp được hỗ trợ trong hệ thống đối với tất cả mô hình dạng Chat (`mode == "chat"`), tự động bổ sung mô hình local mặc định.
12. **Xác thực Registry Schema Động:** Quá trình tải và cập nhật registry giá LiteLLM từ GitHub sử dụng cơ chế xác thực cấu trúc schema động (structural validation - lấy mẫu kiểm tra ngẫu nhiên xem có chứa thông số giá token đầu vào/đầu ra hay không) thay vì kiểm tra sự tồn tại của tên mô hình cứng.
13. **Semantic Caching (Bộ nhớ đệm Ngữ nghĩa):**
    *   Tích hợp cơ chế bộ nhớ đệm ngữ nghĩa sử dụng Redis Stack Vector Search (KNN search) ở cơ sở dữ liệu Redis `DB 0`.
    *   Khi có yêu cầu từ `use_case == "chatbot"`, hệ thống sử dụng thư viện local FastEmbed `multilingual-e5-small` sinh vector embeddings 384 chiều cho câu hỏi.
    *   Thực hiện truy vấn KNN Vector Search trên index `idx:semantic_cache` lọc chính xác theo `tenant_id` và `use_case` để cô lập dữ liệu hoàn toàn.
    *   Nếu độ tương đồng (1 - Cosine Distance) >= 0.92 (Cache Hit) -> Trả về kết quả lưu trữ trực tiếp với độ trễ < 10ms, bỏ qua hoàn toàn việc gọi LangGraph và các nhà cung cấp LLM, ghi nhận Prometheus metric `ai_core_semantic_cache_hits_total`.
    *   Nếu độ tương đồng < 0.92 (Cache Miss) -> Thực hiện luồng ReAct Agent bình thường. Sau khi có kết quả từ LLM, kích hoạt một Celery background task để ghi nhận câu hỏi, câu trả lời cùng vector embeddings vào Redis Stack cache với thời gian sống TTL mặc định là 86,400 giây (24 giờ), ghi nhận Prometheus metric `ai_core_semantic_cache_misses_total`.
...
*   **Sliding Window Memory:** LangGraph tự động nén lịch sử chat của state checkpoint khi vượt quá giới hạn (ví dụ: 10 tin nhắn hoặc 4000 tokens) để giữ token đầu vào tối giản. Hệ thống hỗ trợ hai cơ chế nén:
    1.  *Cơ chế cơ bản (Baseline):* Sử dụng logic cắt chuỗi thô (trích xuất và nối 100 ký tự đầu tiên của các tin nhắn cũ) để làm tóm tắt thô cực nhanh mà không phát sinh thêm chi phí gọi LLM.
    2.  *Cơ chế nâng cao (Production-ready):* Gọi LLM phụ giá rẻ chạy ngầm (ví dụ: `gpt-4o-mini` hoặc model tương đương) để sinh tóm tắt cuộc hội thoại cũ một cách mạch lạc và lưu vào trường `summary` của state.
...
| **FR-AI-009** | Cảnh báo chi phí vượt hạn mức (Cost Alert) | UC-10 | US-069 | BRD §3 | TC-AI-009 | 🟡 Đang dev |
| **FR-AI-010** | Đồng bộ bảng giá LLM động và Cấu hình mặc định hệ thống | UC-10 | US-018 | BRD §3 | TC-AI-010 | 🟡 Đang dev |
| **FR-AI-011** | Phòng vệ mô hình bị khai tử ở Upstream | UC-10 | US-018 | BRD §3 | TC-AI-011 | 🟡 Đang dev |
| **FR-AI-012** | Bộ nhớ đệm ngữ nghĩa (Semantic Caching) qua Redis Stack | UC-10 | US-018 | BRD §3 | TC-AI-012 | 🟡 Đang dev |
