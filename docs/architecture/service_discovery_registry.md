...
### 4.3. Cấu hình Sync Daemon đa dịch vụ (SERVICES_MAP)
Registry Sync Daemon (`sync_registry.py`) sử dụng cấu hình mapping để quản lý nhiều dịch vụ:
```python
SERVICES_MAP = {
...
### 4.4. Mã mẫu Tích hợp NestJS Client (Node.js)
```typescript
...
@Injectable()
export class ServiceRegistryService implements OnModuleInit, OnApplicationShutdown {
  ...
  async onModuleInit() {
    this.redisClient = new Redis({ host: 'redis-master-1', port: 6379 });
    this.ip = await this.getInternalIp();
    await this.register();
    this.heartbeatInterval = setInterval(() => this.heartbeat(), 5000);
  }

  async register() {
    const setKey = `registry:service:${this.serviceName}`;
    const nodeKey = `${setKey}:node:${this.ip}:${this.port}`;
    await this.redisClient.sadd(setKey, `${this.ip}:${this.port}`);
    await this.redisClient.setex(nodeKey, 15, 'alive');
  }
  ...
  async onApplicationShutdown() {
    ...
    const setKey = `registry:service:${this.serviceName}`;
    const nodeKey = `${setKey}:node:${this.ip}:${this.port}`;
    await this.redisClient.srem(setKey, `${this.ip}:${this.port}`);
    await this.redisClient.del(nodeKey);
  }

  private getInternalIp(): Promise<string> {
    return new Promise((resolve) => {
      const s = require('net').createConnection(80, '8.8.8.8');
      s.on('connect', () => {
